...
class coomway {
    ...
    constructor(useDebugMode) {
        this.useDebugMode = useDebugMode
    }

    async search(entry) {

        let selected = musicarray[entry];

        if(!selected) return console.log(chalk.red(`\n\nThe integer provided in your search function is invalid - coomwaymp3\n\n`));

        if(this.useDebugMode) {
            console.log(chalk.blue(`\nDEBUG MODE: ${selected}\n`))
        }

        return `https://cdn.hyperz.dev/coomwaymp3/${selected}`;
    }
